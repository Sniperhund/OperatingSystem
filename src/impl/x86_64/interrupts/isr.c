...
void __attribute__((cdecl)) ISR_Handler(Registers* regs) {
    if (g_ISRHandlers[regs->interrupt] != NULL)
        ...
        kprintf("Unhandled exception %x %s\n", regs->interrupt, g_Exceptions[regs->interrupt]);
        kprintf("  eax=%d  ebx=%d  ecx=%d  edx=%d  edi=%d  esp=%d ebp=%d  eip=%d  eflags=%d  cs=%d  ds=%d  ss=%d",
            regs->eax, regs->ebx, regs->ecx, regs->edx, regs->edi, regs->esp, regs->ebp, regs->eip, regs->eflags, regs->cs, regs->ds, regs->ss);

        kprintf("\n  interrupt=%x  errorcode=%d", regs->interrupt, regs->error);
        //panic();
    }
}
